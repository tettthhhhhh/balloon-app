...
class _CatalogTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final featured = app.featuredProducts.isEmpty
        ? app.products
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          RevealOnMount(
            child: _CatalogHero(app: app, onOpenAssistant: onOpenAssistant),
          ),
          ...
          if (featured.isNotEmpty) ...[
            ...
            ...featured.asMap().entries.map(
              (entry) => RevealOnMount(
                delay: Duration(milliseconds: 100 + (entry.key * 70)),
                child: Padding(
                  padding: const EdgeInsets.only(bottom: 12),
                  child: _ProductCard(product: entry.value, highlight: true),
                ),
              ),
            ),
          ...
          if (app.products.isEmpty)
            ...
            ...app.products.asMap().entries.map(
              (entry) => RevealOnMount(
...
class _CollectionPill extends StatelessWidget {
  const _CollectionPill({required this.icon, required this.label});
    ...
    return HoverLift(
      ...
      child: DecoratedBox(
        decoration: BoxDecoration(
          ...
          border: Border.all(color: AppPalette.peach.withValues(alpha: 0.16)),
        ),
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              Icon(icon, size: 18, color: AppPalette.gold),
...
class _ProductCard extends StatelessWidget {
  const _ProductCard({required this.product, this.highlight = false});
        ...
        children: [
          ...
          if (app.cart.isEmpty)
            const EmptyStatePanel(
              ...
              message:
                  'Добавь позиции из каталога, а потом мы доведём тебя до красивого чек-аута.',
      ...
      body: ListView(
        ...
        children: [
          ...
          const SectionHeading(
            ...
            title: 'Мои заказы',
            subtitle:
                'История заказов и статусы выдачи остаются в одном месте для клиента и команды.',
          ),
          const SizedBox(height: 12),
          if (app.orders.isEmpty)
            const EmptyStatePanel(
              ...
              message:
                  'После первой оплаты здесь появится история, статусы и коды выдачи.',
